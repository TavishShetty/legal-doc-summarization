...
def evaluate_rag(rag: PrivacyRAG, test_data: List[dict]) -> dict:
    ...
    for answer, ground_truth in zip(answers, ground_truths):
        if answer and ground_truth:  # Ensure neither is empty
            scores = rouge.get_scores(answer, ground_truth)[0]  # Get ROUGE-1, ROUGE-2, ROUGE-L
            rouge_scores.append({
                "rouge-1": scores["rouge-1"]["f"],  # F1 score for ROUGE-1
                "rouge-2": scores["rouge-2"]["f"],  # F1 score for ROUGE-2
                "rouge-l": scores["rouge-l"]["f"]   # F1 score for ROUGE-L
            })
        else:
    ...
    combined_result = {
        "ragas": ragas_result,
        "rouge": rouge_scores[0] if rouge_scores else {"rouge-1": 0.0, "rouge-2": 0.0, "rouge-l": 0.0}
    }
...
},
{
    ...
    logger.info(f"Evaluation Results - ROUGE: {evaluation_results['rouge']}")
